    ...
    def sample(self):
        ...
        #     self.sample_sequence(model)
            ...
            x = torch.randn(
                    ...
                    self.config.data.image_size,
                    device=self.device)
            sample_x, _ = self.sample_image(model=model, x=x)
            sample_x = inverse_data_transform(self.config, sample_x)
            for i in range(8):
                tvu.save_image(sample_x[i], os.path.join(self.args.image_folder, f"{i}.png"))
